...
{
    public class Rule47Service : IRule47Service
    {
        ...
        {
            new Rule41ColumnPair { StudCol = "_001", AuditCol = "_001", Label = "Qualification Code" },
            new Rule41ColumnPair { StudCol = "_002", AuditCol = "_002", Label = "Type"               },
            new Rule41ColumnPair { StudCol = "_003", AuditCol = "_003", Label = "Reference"          },
        };

        private readonly IConfiguration _configuration;

        public Rule47Service(IConfiguration configuration)
        {
        ...
        public async Task<Rule41TableDiscoveryResult> GetTablesAsync(string server, string database, string driver)
        {
            try
            {
                ...
                return new Rule41TableDiscoveryResult
                {
                    Success        = true,
                    Tables         = tables,
                    AutoStudTable  = FindFirst(tables, ["QUAL", "dbo_QUAL"],     ["QUAL"]),
                    AutoAuditTable = FindFirst(tables, ["H16QUAL", "H16_QUAL"],  ["H16QUAL", "H16Q"])
                };
            }
        ...
        public async Task<Rule41ColumnDiscoveryResult> GetColumnsAsync(string server, string database, string driver, string tableName)
        {
            try
            {
                ...
                return new Rule41ColumnDiscoveryResult
                {
                    Success = true,
                    Columns = cols,
                    AutoKey = FindFirst(cols, ["_001"], ["_001"])
                };
            }
            ...
            string studTable, string auditTable)
        {
            ...
            foreach (var normKey in allNormKeys.OrderBy(k => k))
            {
                ...
                if (studRow == null)
                {
                    ...
                    row.DisagreeDetail = $"Qualification {display} not in {studTable}";
                }
                else if (auditRow == null)
                {
                    ...
                    row.OverallResult  = "MISSING";
                    row.DisagreeDetail = $"Qualification {display} not in {auditTable}";
                }
                else
                {
                    ...
                    foreach (var p in pairs)
                    {
                        ...
                        if (match == "DISAGREE")
                            diffs.Add($"{p.Label}: QUAL='{sv}' ≠ H16QUAL='{av}'");
                    }
        ...
        public string GenerateSql(Rule41ValidationRequest request)
        {
            ...
            var fieldSel  = BuildFieldSelect(pairs, "q", "h");
            var agreeCond = BuildAgreeCondition(pairs, "q", "h");
            var testedCols = string.Join(", ", pairs.Select(p => $"{p.StudCol}↔{p.AuditCol}"));

            return $@"-- ============================================================
-- HEMIS RULE 47 – QUAL vs H16QUAL Agreement
...
IF OBJECT_ID('tempdb..#Rule47') IS NOT NULL DROP TABLE #Rule47;

SELECT
    ROW_NUMBER() OVER (ORDER BY COALESCE(q.[{sk}], h.[{ak}])) AS Row_No,
    COALESCE(q.[{sk}], h.[{ak}]) AS Qual_Ref,
{fieldSel},
    CASE
        WHEN q.[{sk}] IS NULL THEN 'MISSING-QUAL'
        WHEN h.[{ak}] IS NULL THEN 'MISSING-H16QUAL'
        WHEN {agreeCond} THEN 'AGREE'
        ELSE 'DISAGREE'
    END AS Overall_Result
INTO #Rule47
FROM [{st}] q
FULL OUTER JOIN [{at}] h
    ON UPPER(LTRIM(RTRIM(CAST(q.[{sk}] AS NVARCHAR(200)))))
     = UPPER(LTRIM(RTRIM(CAST(h.[{ak}] AS NVARCHAR(200)))));
...
SELECT * FROM #Rule47 ORDER BY Row_No;
SELECT * FROM #Rule47 WHERE Overall_Result <> 'AGREE' ORDER BY Row_No;
...
-- ============================================================".Trim();
        ...
        private async Task<int> SaveValidationRunAsync(Rule41ValidationRequest request, Rule41ValidationSummary summary, string? userEmail, string? userName)
        {
            ...
            await MarkPreviousRunsHistoricalAsync(connection, request.ClientId, 47);
            ...
            var previousHash = await GetLatestValidationHashAsync(connection, request.ClientId, 47);
...
VALUES
(
    @ClientID, @UserID, 47, 'QUAL vs H16QUAL Agreement', @Status, @TotalRecords, @PassCount, @FailCount, @ExceptionRate, GETDATE(),
    ...
    NULL, @ResultsJSON, @RunByUserName, NULL, NULL, @PreviousHash, NULL, 1
            ...
            hashCmd.Parameters.AddWithValue("@RecordHash", ComputeHash($"Rule47|{runId}|{request.ClientId}|{summary.Status}|{summary.TotalCount}|{summary.ExceptionRate}|{summary.Timestamp}|{previousHash}"));
...
SELECT TOP 1
    ...
    ISNULL(vr.StudTable,'QUAL')         AS StudTable,
    ISNULL(vr.DeceasedTable,'H16QUAL')  AS AuditTable,
    ISNULL(vr.StudColumn,'_001')        AS StudKey,
    ISNULL(vr.DeceasedColumn,'_001')    AS AuditKey,
    ISNULL(vr.Status,'')                AS Status,
    vr.LastEditedByUserName, vr.LastEditedAt, vr.ResultsJSON
FROM dbo.ValidationRuns vr
WHERE vr.ClientID = @ClientID AND vr.RuleNumber = 47 AND vr.IsCurrent = 1
ORDER BY vr.RunTimestamp DESC, vr.RunID DESC;";
            ...
            var workspace = new Rule41WorkspaceStateViewModel
            {
                ...
                StudTable            = reader.IsDBNull(4)  ? "QUAL"    : reader.GetString(4),
                AuditTable           = reader.IsDBNull(5)  ? "H16QUAL" : reader.GetString(5),
                StudKey              = reader.IsDBNull(6)  ? "_001"    : reader.GetString(6),
                AuditKey             = reader.IsDBNull(7)  ? "_001"    : reader.GetString(7),
...
WHERE vr.RunID = @RunID AND vr.RuleNumber = 47;";
...
WHERE RunID = @RunID AND ClientID = @ClientID;";
                ...
                command.Parameters.AddWithValue("@PreviousHash",        (object?)previousHash ?? DBNull.Value);
                command.Parameters.AddWithValue("@RecordHash",          ComputeHash($"WorkspaceSave|Rule47|{request.RunId.Value}|{request.ClientId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
        ...
        public async Task<Rule41WorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                ...
                await MarkPreviousRunsHistoricalAsync(connection, clientId.Value, 47);
...
WHERE RunID = @RunID;";
                ...
                markEdit.Parameters.AddWithValue("@RecordHash",         ComputeHash($"BeginEdit|Rule47|{runId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
