...
ORDER BY vr.IsCurrent DESC, vr.RunTimestamp DESC, vr.RunID DESC;";
        ...
        public async Task<ClinicalTechValidationSummary?> GetFullSummaryByRunIdAsync(int runId)
        {
            try
            {
                ...
                return JsonConvert.DeserializeObject<ClinicalTechValidationSummary>(ValidationPayloadCodec.Decode(val.ToString()!));
            }
            catch { return null; }
        }
