...
{
    public class Rule48Service : IRule48Service
    {
        ...
        {
            new Rule41ColumnPair { StudCol = "_001", AuditCol = "_001", Label = "Credential Code" },
            new Rule41ColumnPair { StudCol = "_030", AuditCol = "_030", Label = "Course Code"     },
            new Rule41ColumnPair { StudCol = "_036", AuditCol = "_036", Label = "_036"            },
            new Rule41ColumnPair { StudCol = "_065", AuditCol = "_065", Label = "_065"            },
        };

        private readonly IConfiguration _configuration;

        public Rule48Service(IConfiguration configuration)
        {
        ...
        public async Task<Rule41TableDiscoveryResult> GetTablesAsync(string server, string database, string driver)
        {
            try
            {
                ...
                return new Rule41TableDiscoveryResult
                {
                    Success        = true,
                    Tables         = tables,
                    AutoStudTable  = FindFirst(tables, ["dbo_CRED", "CRED"],       ["CRED"]),
                    AutoAuditTable = FindFirst(tables, ["H16CRED",  "H16_CRED"],   ["H16CRED", "H16CR"])
                };
            }
            ...
            string studTable, string auditTable)
        {
            ...
            foreach (var normKey in allNormKeys.OrderBy(k => k))
            {
                ...
                if (studRow == null)
                {
                    ...
                    row.DisagreeDetail = $"Credential {display} not in {studTable}";
                }
                else if (auditRow == null)
                {
                    ...
                    row.OverallResult  = "MISSING";
                    row.DisagreeDetail = $"Credential {display} not in {auditTable}";
                }
                else
                {
                    ...
                    foreach (var p in pairs)
                    {
                        ...
                        if (match == "DISAGREE")
                            diffs.Add($"{p.Label}: CRED='{sv}' ≠ H16CRED='{av}'");
                    }
        ...
        public string GenerateSql(Rule41ValidationRequest request)
        {
            ...
            var fieldSel   = BuildFieldSelect(pairs, "c", "h");
            var agreeCond  = BuildAgreeCondition(pairs, "c", "h");
            var testedCols = string.Join(", ", pairs.Select(p => $"{p.StudCol}↔{p.AuditCol}"));

            return $@"-- ============================================================
-- HEMIS RULE 48 – CRED vs H16CRED Agreement
...
IF OBJECT_ID('tempdb..#Rule48') IS NOT NULL DROP TABLE #Rule48;

SELECT
    ROW_NUMBER() OVER (ORDER BY COALESCE(c.[{sk}], h.[{ak}])) AS Row_No,
    COALESCE(c.[{sk}], h.[{ak}]) AS Cred_Ref,
{fieldSel},
    CASE
        WHEN c.[{sk}] IS NULL THEN 'MISSING-CRED'
        WHEN h.[{ak}] IS NULL THEN 'MISSING-H16CRED'
...
FULL OUTER JOIN [{at}] h
    ON UPPER(LTRIM(RTRIM(CAST(c.[{sk}] AS NVARCHAR(200)))))
...
SELECT * FROM #Rule48 ORDER BY Row_No;
SELECT * FROM #Rule48 WHERE Overall_Result <> 'AGREE' ORDER BY Row_No;
...
-- ============================================================".Trim();
        ...
        private async Task<int> SaveValidationRunAsync(Rule41ValidationRequest request, Rule41ValidationSummary summary, string? userEmail, string? userName)
        {
            ...
            await MarkPreviousRunsHistoricalAsync(connection, request.ClientId, 48);
            ...
            var previousHash = await GetLatestValidationHashAsync(connection, request.ClientId, 48);
...
VALUES
(
    @ClientID, @UserID, 48, 'CRED vs H16CRED Agreement', @Status, @TotalRecords, @PassCount, @FailCount, @ExceptionRate, GETDATE(),
    ...
    NULL, @ResultsJSON, @RunByUserName, NULL, NULL, @PreviousHash, NULL, 1
            ...
            hashCmd.Parameters.AddWithValue("@RecordHash", ComputeHash($"Rule48|{runId}|{request.ClientId}|{summary.Status}|{summary.TotalCount}|{summary.ExceptionRate}|{summary.Timestamp}|{previousHash}"));
...
SELECT TOP 1
    ...
    ISNULL(vr.StudTable,'dbo_CRED')      AS StudTable,
    ISNULL(vr.DeceasedTable,'H16CRED')   AS AuditTable,
...
WHERE vr.ClientID = @ClientID AND vr.RuleNumber = 48 AND vr.IsCurrent = 1
ORDER BY vr.RunTimestamp DESC, vr.RunID DESC;";
            ...
            var workspace = new Rule41WorkspaceStateViewModel
            {
                ...
                AuditTable           = reader.IsDBNull(5)  ? "H16CRED"  : reader.GetString(5),
...
WHERE vr.RunID = @RunID AND vr.RuleNumber = 48;";
...
WHERE RunID = @RunID AND ClientID = @ClientID;";
                ...
                command.Parameters.AddWithValue("@PreviousHash",        (object?)previousHash ?? DBNull.Value);
                command.Parameters.AddWithValue("@RecordHash",          ComputeHash($"WorkspaceSave|Rule48|{request.RunId.Value}|{request.ClientId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
        ...
        public async Task<Rule41WorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                ...
                await MarkPreviousRunsHistoricalAsync(connection, clientId.Value, 48);
...
WHERE RunID = @RunID;";
                ...
                markEdit.Parameters.AddWithValue("@RecordHash",          ComputeHash($"BeginEdit|Rule48|{runId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
