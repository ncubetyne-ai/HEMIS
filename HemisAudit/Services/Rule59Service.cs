...
namespace HemisAudit.Services
{
    public class Rule59Service : IRule59Service
    {
        ...
        public Rule59Service(IConfiguration configuration)
        {
        ...
        public async Task<Rule59TableDiscoveryResult> GetTablesAsync(string server, string database, string driver)
        {
            try
            {
                ...
                return new Rule59TableDiscoveryResult
                {
                    ...
                    AutoValpacTable = FindFirst(tables,
                        ["dbo_SFTE_VALPAC", "SFTE_VALPAC"],
                        ["sfte_valpac", "sftevalpac"]),
        ...
        public async Task<Rule59VerifyResult> VerifyTablesAsync(Rule59VerifyRequest request)
        {
            try
            {
        ...
        public async Task<Rule59ValidationSummary> RunValidationAsync(Rule59ValidationRequest request, string? userEmail = null, string? userName = null)
        {
            try
            {
                ...
                if (!browserSummary.SavedRunId.HasValue)
                    ...
                    browserSummary.Warning = "The current Rule 59 run has been written to the system database. Click Save Workspace to finalize it for signoff.";
        ...
        public async Task<Rule59WorkspaceStateViewModel?> GetCurrentWorkspaceStateAsync(int clientId, string? currentUserEmail = null, bool includeSummary = true)
        {
            await using var connection = await OpenSystemConnectionAsync();
...
WHERE vr.ClientID = @ClientID AND vr.RuleNumber = 59 AND vr.IsCurrent = 1
ORDER BY vr.RunTimestamp DESC, vr.RunID DESC;";
            ...
            var valpacTable          = reader.IsDBNull(4) ? "dbo_SFTE_VALPAC" : reader.GetString(4);
        ...
        public async Task<Rule59RunReviewViewModel?> GetSavedRunAsync(int runId, string? currentUserEmail = null, bool includeFullResults = false)
        {
            await using var connection = await OpenSystemConnectionAsync();
            var currentUserId = await GetSystemUserIdByEmailAsync(connection, currentUserEmail);
...
WHERE vr.RunID = @RunID AND vr.RuleNumber = 59;";
            ...
            var review = new Rule59RunReviewViewModel
            {
        ...
        public async Task<Rule59WorkspaceSaveResult> SaveWorkspaceAsync(Rule59ValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                if (request.RunId is null || request.RunId <= 0)
                    return new Rule59WorkspaceSaveResult { Success = false, Error = "Run Rule 59 first so the workspace can be saved." };

                await using var connection = await OpenSystemConnectionAsync();
                var clientId = await GetClientIdForRunAsync(connection, request.RunId.Value);
                if (!clientId.HasValue || clientId.Value != request.ClientId)
                    return new Rule59WorkspaceSaveResult { Success = false, Error = "The saved workspace could not be found for this engagement." };
...
WHERE RunID=@RunID AND ClientID=@ClientID;";
                ...
                command.Parameters.AddWithValue("@RecordHash", ComputeHash($"WorkspaceSave|Rule59|{request.RunId.Value}|{request.ClientId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
        ...
        public async Task<Rule59WorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                ...
                if (!clientId.HasValue) return new Rule59WorkspaceSaveResult { Success = false, Error = "Saved workspace was not found." };
...
WHERE RunID=@RunID;";
                ...
                markEdit.Parameters.AddWithValue("@RecordHash", ComputeHash($"BeginWorkspaceEdit|Rule59|{runId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
...
ELSE
    INSERT INTO dbo.ReviewSignoffs (ClientID,RunID,ReviewerID,SignoffRole,ReviewType,Comment,SignedOffAt) VALUES (@ClientID,@RunID,@ReviewerID,@SignoffRole,'Final',@Comment,GETDATE());";
        ...
        public Task<string> GenerateSqlAsync(Rule59ValidationRequest request)
        {
            ValidateRequest(request);
            var valpacTable = Sanitise(request.ValpacTable);
            var prodTable   = Sanitise(request.ProdTable);
            ...
            var sql = $@"-- HEMIS RULE 59: SFTE VALPAC DATA IN STAFF PRODUCTION
...
SELECT
    ...
    (SELECT COUNT(1) FROM ValidationResults WHERE PROD_PERSONEL_NUMBER IS NULL)     AS Missing;";
        ...
        private async Task<Rule59ValidationSummary> AnalyseAsync(Rule59ValidationRequest request, bool includeAllReviewRows)
        {
            var connStr = BuildConnectionString(request.Server, request.Database, request.Driver);
            await using var conn = new SqlConnection(connStr);
            await conn.OpenAsync();
            ...
            return new Rule59ValidationSummary
            {
                ...
                Warning = includeAllReviewRows
                    ? "Rule 59 completed with the full VALPAC population result set."
...
WITH ValpacData AS
(
    SELECT CONVERT(nvarchar(255), V.[{v037}]) AS VALPAC__037
    FROM [{valpacTable}] V
...
ORDER BY {orderBy};";
        ...
        private async Task<List<Rule59ValidationRowRecord>> LoadRowsAsync(SqlConnection connection, string valpacTable, string prodTable, int? maxRows, string v037, string pPersonel)
        {
            await using var command = connection.CreateConfiguredCommand();
        ...
        private static List<Rule59ControlSummaryItemViewModel> BuildControlSummaries(int total, int matched, string valpacTable, string prodTable)
        {
            ...
            return new List<Rule59ControlSummaryItemViewModel>
            {
                new()
                {
                    ...
                    TotalCount   = total, PassCount = matched, FailCount = fail,
        ...
        private async Task<int> SaveValidationRunAsync(Rule59ValidationRequest request, Rule59ValidationSummary summary, string? userEmail, string? userName, bool markWorkspaceSaved)
        {
            await using var connection = await OpenSystemConnectionAsync();
            await EnsureClientNotArchivedAsync(connection, request.ClientId);
            await MarkPreviousRunsHistoricalAsync(connection, request.ClientId, 59);
            ...
            var previousHash = await GetLatestValidationHashAsync(connection, request.ClientId, 59);
...
VALUES
(@ClientID,@UserID,59,@RuleName,@Status,@TotalRecords,@PassCount,@FailCount,@ExceptionRate,GETDATE(),
...
SELECT CAST(SCOPE_IDENTITY() AS int);";
            ...
            cmd.Parameters.AddWithValue("@RuleName", "SFTE VALPAC Data in STAFF PRODUCTION");
            ...
            hashCmd.Parameters.AddWithValue("@RecordHash", ComputeHash($"ValidationRun|Rule59|{runId}|{request.ClientId}|{systemUserId.Value}|{summary.Status}|{summary.TotalValidated}|{summary.FailCount}|{summary.Timestamp}|{previousHash}"));
        ...
        private static void ApplyBrowserPreview(Rule59ValidationSummary summary)
        {
            ...
            var selected = new List<Rule59ValidationRowRecord>();
        ...
        private static Rule59ValidationSummary CloneSummary(Rule59ValidationSummary s) => new()
        {
            ...
            ControlSummaries = s.ControlSummaries.Select(i => new Rule59ControlSummaryItemViewModel
            {
            ...
            ReviewRows = s.ReviewRows.Select(r => new Rule59ValidationRowRecord
            {
        ...
        private async Task<Rule59ValidationSummary> ExpandSummaryIfNeededAsync(Rule59ValidationSummary summary, string? server)
        {
            ...
            try
            {
                var expanded = await AnalyseAsync(new Rule59ValidationRequest
                {
        ...
        private static void ValidateRequest(Rule59ValidationRequest request)
        {
        ...
        private static void ValidateRequest(Rule59VerifyRequest request)
        {
            if (string.IsNullOrWhiteSpace(request.Server)) throw new InvalidOperationException("Server name is required.");
            if (string.IsNullOrWhiteSpace(request.Database)) throw new InvalidOperationException("Database is required.");
            if (string.IsNullOrWhiteSpace(request.ValpacTable)) throw new InvalidOperationException("VALPAC table is required.");
...
ORDER BY CASE ISNULL(rs.SignoffRole,'') WHEN 'DataAnalyst' THEN 1 WHEN 'Manager' THEN 2 WHEN 'Director' THEN 3 ELSE 4 END, rs.SignedOffAt DESC;";
        ...
        private static async Task UpdateStoredSummaryAsync(SqlConnection connection, int runId, Rule59ValidationSummary summary)
        {
            await using var cmd = connection.CreateConfiguredCommand();
        ...
        private static Rule59ValidationSummary? DeserializeSummary(string? json)
        {
            ...
            try
            {
                ...
                return string.IsNullOrWhiteSpace(decoded) ? null : JsonConvert.DeserializeObject<Rule59ValidationSummary>(decoded);
            }
            catch { return null; }
        }
        ...
        private static List<Rule59ValidationRowRecord> NormalizeRows(IEnumerable<Rule59ValidationRowRecord> rows) =>
