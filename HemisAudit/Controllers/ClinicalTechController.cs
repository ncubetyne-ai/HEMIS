    ...
    public class ClinicalTechController : Controller
    {
        ...
        public async Task<IActionResult> SaveWorkspace([FromBody] ClinicalTechValidationRequest request)
        {
            ...
            if (result)
            {
                ...
                var workspace = await _clinicalTech.GetCurrentWorkspaceStateAsync(request.ClientId, user?.Email);
                var resultsVisible = CanViewWorkspaceResults(role, workspace);
                if (workspace != null) workspace.ResultsVisible = resultsVisible;
                if (workspace != null && !resultsVisible) workspace.Summary = null;
        ...
        public async Task<IActionResult> BeginEdit([FromBody] QualSurnameSignoffInput model)
        {
            ...
            var role = await GetCurrentSystemRoleAsync(user);
            var workspace = await _clinicalTech.GetCurrentWorkspaceStateAsync(model.ClientId, user.Email);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
            if (workspace != null) workspace.ResultsVisible = resultsVisible;
            if (workspace != null && !resultsVisible) workspace.Summary = null;
        ...
        {
            var summary = await _clinicalTech.GetFullSummaryByRunIdAsync(model.RunId);
        ...
        public async Task<IActionResult> DownloadCsv([FromBody] QualSurnameSignoffInput model)
        {
            ...
            return File(bytes, "text/csv", "Rule69_ClinicalTech.csv");
        }
        ...
        public async Task<IActionResult> DownloadSql([FromBody] ClinicalTechValidationRequest request)
        {
            ...
            return File(bytes, "text/plain", "Rule69_ClinicalTech.sql");
        }
