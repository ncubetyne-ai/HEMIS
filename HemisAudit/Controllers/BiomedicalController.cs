    ...
    public class BiomedicalController : Controller
    {
        ...
        public async Task<IActionResult> SaveWorkspace([FromBody] BiomedicalValidationRequest request)
        {
            ...
            if (result)
            {
                ...
                var workspace = await _biomedical.GetCurrentWorkspaceStateAsync(request.ClientId, user?.Email);
                var resultsVisible = CanViewWorkspaceResults(role, workspace);
                if (workspace != null) workspace.ResultsVisible = resultsVisible;
                if (workspace != null && !resultsVisible) workspace.Summary = null;
        ...
        public async Task<IActionResult> BeginEdit([FromBody] QualSurnameSignoffInput model)
        {
            ...
            if (user == null) return Json(new { success = false, error = "Not authenticated." });
            var role = await GetCurrentSystemRoleAsync(user);
            var workspace = await _biomedical.GetCurrentWorkspaceStateAsync(model.ClientId, user.Email);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
            if (workspace != null) workspace.ResultsVisible = resultsVisible;
            if (workspace != null && !resultsVisible) workspace.Summary = null;
        ...
        {
            var summary = await _biomedical.GetFullSummaryByRunIdAsync(model.RunId);
        ...
        public async Task<IActionResult> DownloadCsv([FromBody] QualSurnameSignoffInput model)
        {
            ...
            return File(bytes, "text/csv", "Rule74_Biomedical.csv");
        }
        ...
        public async Task<IActionResult> DownloadSql([FromBody] BiomedicalValidationRequest request)
        {
            ...
            return File(bytes, "text/plain", "Rule74_Biomedical.sql");
        }
